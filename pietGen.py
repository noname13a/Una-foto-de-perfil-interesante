...
def draw_block(size,num):
	...
	pos_y = 12*num
	pos_x = 0
	piet[pos_x:pos_x+12,pos_y:pos_y+12] = block
...
def draw_end(num):
	...
	pos_y = 12*num
	pos_x = 0
	piet[pos_x:pos_x+12,pos_y:pos_y+5] = block


current_color = Color()
piet = []
message = input("Introduce la flag: \n")
painting_len = len(message)*12 + 5
piet = np.zeros((12,painting_len,3), dtype=np.uint8)
